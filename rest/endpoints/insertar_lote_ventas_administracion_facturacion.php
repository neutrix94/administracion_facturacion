<?php
    ...
    $app->post('/insertar_lote_ventas_administracion_facturacion', function (Request $request, Response $response, $args) {
        ...
        if( ! isset( $req['ventas'] ) ){
            $response->getBody()->write(json_encode( array( "status"=>"400", "message"=>"El parametro ventas es requerido" ) ));
            return $response;
        }
        $ventas = $req['ventas'];
    ...
    //consulta si la venta ya exista
            ...
            try{
                ...
                die( json_encode( array( "status"=>"302", "message"=>"Error al consultar si la venta ya exista,", "query"=>"{$sql}", "error_detail"=>"{$error->getMessage()}") ) );
            }
            $link->beginTransaction();//inicio de transaccion
            if( $stm->rowCount() <= 0 ){//si la venta no existe la inserta
                try{
                ...
                //recupera id inserado
                    try{
                        ...
                        $stm2 = $link->query( $sql ) or die( "Error al consultar id de cabecera de venta insertada : {$sql}" );
                        $row = $stm2->fetch(PDO::FETCH_ASSOC);
                        $sale_id = $row['last_id'];
                    }catch(PDOException $error){
                        die( json_encode( array( "status"=>"302", "message"=>"Error al consultar el id de venta insertado : {$sql}", "error_detail"=>"{$error->getMessage()}") ) );
                    }
                }catch(PDOException $error){
                    die( json_encode( array( "status"=>"302", "message"=>"Error al insertar cabecera de movimiento de almacen : {$sql}", "error_detail"=>"{$error->getMessage()}") ) );
                }
            }else{//si la venta existe actualiza
                $row = $stm->fetch(PDO::FETCH_ASSOC);
                $sale_id = $row['id_pedido'];
                $sql = "UPDATE ec_pedidos SET folio_pedido = '{$venta['folio_pedido']}', folio_nv = '{$venta['']}', folio_factura = '{$venta['']}', folio_cotizacion = '{$venta['']}', 
                            id_cliente = '{$venta['']}', id_estatus = '{$venta['']}', id_moneda = '{$venta['']}', fecha_alta = '{$venta['']}', fecha_factura = '{$venta['']}', 
                            id_direccion = '{$venta['']}', direccion = '{$venta['']}', id_razon_social = '{$venta['']}', subtotal = '{$venta['']}', iva` = '{$venta['']}', 
                            ieps = '{$venta['']}', total = '{$venta['']}', dias_proximo = '{$venta['']}', pagado = '[{$venta['']}', surtido = '{$venta['']}', 
                            enviado = '{$venta['']}', id_sucursal = '{$venta['']}', id_usuario = '{$venta['']}', fue_cot = '{$venta['']}', facturado = '{$venta['']}', id_tipo_envio = '{$venta['']}', 
                            descuento = '{$venta['']}', id_razon_factura = '{$venta['']}', folio_abono = '{$venta['']}', correo = '{$venta['']}', facebook = '{$venta['']}', modificado = '{$venta['']}', 
                            ultima_sincronizacion = '{$venta['']}', ultima_modificacion = '{$venta['']}', tipo_pedido = '{$venta['']}', id_status_agrupacion = '{$venta['']}', 
                            id_cajero = '{$venta['']}', id_devoluciones = '{$venta['']}', venta_validada = '{$venta['']}', folio_unico = '{$venta['']}', id_sesion_caja = '{$venta['']}', 
                            tipo_sistema = '{$venta['']}', monto_pago_inicial = '{$venta['']}', cobro_finalizado = '{$venta['']}', id_status_facturacion = '{$venta['']}' 
