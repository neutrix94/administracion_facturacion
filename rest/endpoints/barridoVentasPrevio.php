<?php
    ...
    $app->post('/barrido_ventas_previo', function (Request $request, Response $response, $args) {
       // die( "here" );
        ...
        $responseFacturacion["rows"] = array();
        $date_since = "";
        $date_to = "";
        $rs_id = "-1";
        $sql_condition = "";
...
//consulta ventas pendientes de mandar a Razones Sociales (status 4 o menor)
        ...
        //itera las razones sociales
            ...
            //consulta cabeceras
                $sqlGetVenta = "SELECT 
                        ...
                        {$sql_condition}
                        LIMIT {$RS_row['rows_limit']}";//die($sqlGetVenta);
            ...
            //itera resultados de las ventas
                ...
                //consulta pagos
                    ...
                    array_push( $RS_row['sales'], $sale );
                    //$post_data = json_encode( array( "sales"=>$sales ) );//forma JSON
                }
                array_push( $requestRS, $RS_row );
                /*if( sizeof( $sales ) > 0 ){
        ...
        $response->getBody()->write(json_encode( $response_msg ));
