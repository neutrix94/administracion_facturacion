...
//descarga de csv
	...
	//generamos descarga
		...
		echo mb_convert_encoding($info, "ISO-8859-1", "UTF-8");
...
//		$field = mysqli_num_fields($eje);
		...
		$numColumns = $eje->columnCount();
    
		// Obtener los nombres de las columnas
		for ($i = 0; $i < $numColumns; $i++) {
			$columnMeta = $eje->getColumnMeta($i);  // Obtener los metadatos de la columna
			$names[$i] = $columnMeta['name'];  // Guardar el nombre de la columna
		}
		...
		$c=0;
		while( $r = $eje->fetch(PDO::FETCH_NUM) ){
			if($c==0){
